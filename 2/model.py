...
import pytorch # библиотека для работы с нейронными сетями
import numpy as np # библиотека для работы с числовыми данными

def train_channel_model(data):
  # функция для обучения модели для выбора канала размещения рекламы
...
def train_bid_model(data):
  # функция для обучения модели для выбора ставки для рекламы
  # принимает на вход данные о пользовательском поведении и рынке
  ...
  # разделение данных на обучающую и тестовую выборки
  X = data[:, :-1] # все признаки, кроме последнего (став
